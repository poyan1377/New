...
async def webapp_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # بررسی می‌کنیم که آیا update شامل web_app_data هست یا خیر
    if update.message and update.message.web_app_data:
        ...
        if data == "accepted":
            ...
            await update.message.reply_text(
                f"کاربر {user.first_name} شما قوانین را تایید کردید 🙂 هم‌اکنون می‌توانید حساب خود را شارژ کنید 🤩"
            )
        elif data == "rejected":
            await update.message.reply_text(
                "شما قوانین را رد کردید. امکان شارژ حساب وجود ندارد.", 
                quote=True
            )

# Added functions to handle terms acceptance via callback query.
async def accept_terms_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    TERMS_ACCEPTED[user.id] = True
    await update.callback_query.edit_message_text("قوانین تایید شد. می‌توانید ادامه دهید!")

async def reject_terms_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.callback_query.edit_message_text("قوانین تایید نشد. لطفاً دوباره تلاش کنید!")
...
if __name__ == '__main__':
    async def main():
        ...
        application.add_handler(MessageHandler(filters.ALL, webapp_data_handler))
